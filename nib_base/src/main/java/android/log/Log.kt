    ...
    val stack: StackTraceElement
        get() {
            ...
            while (i < N) {
                ...
                if (className.matches(LOG_CLASS.toRegex())) {
                    i++
                    continue
                }
                break
...
//        return if (MODE == eMODE.ECLIPSE)
//            String.format(Locale.getDefault(), ":at (%s:%d)", info.fileName, info.lineNumber)//eclipse
//        else
        return String.format(Locale.getDefault(), "(%s:%d)", info.fileName, info.lineNumber)//android studio
    }
...
//        var byte= byte_text[byte_start_index]
//        byte and 0xc0.toByte()
        if (byte_text[byte_start_index] and 0xc0.toByte() == 0x80.toByte())
        ...
        while (byte_start_index <= po) {
            if (byte_text[po] and 0xc0.toByte() != 0x80.toByte())
                break
            po--
        }
    ...
    fun e_filter(sec: Long, vararg args: Any): Int {
        if (!LOG)
            return -1
    ...
    fun clz(clz: Class<*>): Int {
        ...
        retult += i("getPackage", clz.`package`)
        ...
        if (clz.isInstance(CharSequence::class.java)) retult += i(clz.isInstance(CharSequence::class.java), "isInstance")
        if (clz.isAssignableFrom(CharSequence::class.java!!)) retult += i(clz.isAssignableFrom(CharSequence::class.java!!), "isAssignableFrom")
    ...
    fun _MESSAGE(vararg args: Any?): String {
        ...
        for (obj in args) {
            try {
                ...
                else if (obj is Class<*>         ) sb.append(_DUMP(obj)           )
                else if (obj is Cursor           ) sb.append(_DUMP(obj)           )
                else if (obj is View             ) sb.append(_DUMP(obj)           )
                else if (obj is Intent           ) sb.append(_DUMP(obj)           )
                else if (obj is Bundle           ) sb.append(_DUMP(obj)           )
                else if (obj is ContentValues    ) sb.append(_DUMP(obj)           )
                else if (obj is Throwable        ) sb.append(_DUMP(obj)           )
                else if (obj is Uri              ) sb.append(_DUMP(obj)           )
                else if (obj is Method           ) sb.append(_DUMP(obj)           )
                else if (obj is JSONObject       ) sb.append(obj.toString(2)      )
                else if (obj is JSONArray        ) sb.append(obj.toString(2)      )
                else if (obj is CharSequence     ) sb.append(_DUMP(obj.toString()))
                else if (obj.javaClass.isArray() ) sb.append(_DUMP_array(obj)     )
    ...
    private fun _DUMP(v: View, depth: Int = 0): String {
        ...
        if (v is WebView)
            ...
            out.insert(depth, "N:" + Integer.toHexString(System.identityHashCode(v)) + ":" + v.javaClass.getSimpleName())
    ...
    private fun _DUMP(values: ContentValues?): String {
        ...
        for ((key, value1) in values.valueSet()) {
            ...
            val type = value1.javaClass.getSimpleName()
    ...
    private fun _DUMP(bundle: Bundle?): String {
        ...
        for (key in keys) {
            ...
            if (o == null) {
                ...
                sb.append(o.javaClass.getSimpleName() + " " + key + ";//" + _DUMP_array(o))
            } else {
                sb.append(o.javaClass.getSimpleName() + " " + key + ";//" + o.toString())
            }
            sb.append("\n")
        }
...
//		return cls.getSimpleName() + ((cls.getSuperclass() != null) ? (">>" + cls.getSuperclass().getSimpleName()) : "");
    ...
    private fun _DUMP(uri: Uri?): String {
        ...
        sb.append("\r\n Scheme                    ").append(if (uri.scheme != null) uri.scheme.toString() else "null")
        sb.append("\r\n Host                      ").append(if (uri.host != null) uri.host.toString() else "null")
        //        sb.append("\r\n Port                      ").append(uri.getPort());
        sb.append("\r\n Path                      ").append(if (uri.path != null) uri.path.toString() else "null")
        sb.append("\r\n Query                     ").append(if (uri.query != null) uri.query.toString() else "null")
        //        sb.append("\r\n");
        sb.append("\r\n Fragment                  ").append(if (uri.fragment != null) uri.fragment.toString() else "null")
...
//
        ...
        val sb = StringBuilder()
        if (intent.action != null) if (sb.isNotBlank()) sb.append("\n"); sb.append("Action     " + intent.action)
        if (intent.data != null) if (sb.isNotBlank()) sb.append("\n"); sb.append("Data       " + intent.data)
        if (intent.categories != null) if (sb.isNotBlank()) sb.append("\n"); sb.append("Categories " + intent.categories)
        if (intent.type != null) if (sb.isNotBlank()) sb.append("\n"); sb.append("Type       " + intent.type)
        if (intent.scheme != null) if (sb.isNotBlank()) sb.append("\n"); sb.append("Scheme     " + intent.scheme)
        if (intent.`package` != null) if (sb.isNotBlank()) sb.append("\n"); sb.append("Package    " + intent.`package`)
        if (intent.component != null) if (sb.isNotBlank()) sb.append("\n"); sb.append("Component  " + intent.component)
        if (intent.flags != 0x00) if (sb.isNotBlank()) sb.append("\n"); sb.append("Flags      " + Integer.toHexString(intent.flags))
        //@formatter:on
    ...
    fun _DUMP(th: Throwable): String {
        ...
        try {
            ...
            while (cause != null) {
                message = cause.javaClass.getSimpleName() + "," + cause.message
                cause = cause.cause
            }
        } catch (e: Exception) {
        }
    ...
    private fun _DUMP_object(name: String, value: Any?, duplication: MutableSet<Any>): String {
        ...
        try {
            ...
            if (value.javaClass.isArray()) {
                sb.append(name).append('<').append(value.javaClass.getSimpleName()).append('>').append(" = ")
                //@formatter:off
 ...
 //@formatter:on
            ...
            //
            {
                ...
                if (value is Collection<*>) {
                    sb.append(name).append('<').append(value.javaClass.getSimpleName()).append('>').append(" = ").append(":\n")
                    ...
                    sb.append(name).append('<').append(value.javaClass.getSimpleName()).append('>').append(" = ").append(":\n")
                    for (f in clz.getDeclaredFields()) {
    ...
    fun tic() {
        ...
        e(String.format(Locale.getDefault(), "%15d%15d%15d", s, e, e - s))
        SEED_S[seed] = e
    }
    ...
    fun startActivityForResult(clz: Class<*>, intent: Intent, requestCode: Int) {
        ...
        try {
            val target = if (intent.component != null) intent.component!!.shortClassName else intent.toUri(0)
            Log.pc(Log.ERROR, if (requestCode == -1) "startActivity" else "startActivityForResult", "▶▶", clz, target, intent, requestCode)
            //		printStackTrace();
        } catch (e: Exception) {
        }
    ...
    fun startActivityForResult(clz: Class<*>, intent: Intent, requestCode: Int, options: Bundle?) {
        if (!LOG)
            return
        try {
            val target = if (intent.component != null) intent.component.shortClassName else intent.toUri(Intent.URI_INTENT_SCHEME)
            Log.pc(Log.ERROR, if (requestCode == -1) "startActivity" else "startActivityForResult", "▶▶", clz, target, intent, (options
                    ?: ""), requestCode)
            //		printStackTrace();
        } catch (e: Exception) {
        }
    ...
    fun printStackTrace() {
        if (!LOG)
            return
        에러아님_Exception().printStackTrace()
    }
